...
1. Artificial Potential Fields (APF)The robot moves based on the resultant force vector $F_{total} = F_{att} + F_{rep}$.
   Attractive Force: Pulls the robot towards the goal using the equation
          $U_{att} = k_{att} \times distance$ with a gain of $k_{att} = 1.5$.
   Repulsive Force: Pushes the robot away from obstacles using a repulsive gain
         $k_{rep} = 50.0$ and an influence range of $d_0 = 5.0$.2.
...
📈 📈 Results
The system was tested on 3 complex scenarios achieving a 100% Success Rate:

Scenario 1 (Narrow Gaps): Successfully navigated through vertical walls with a path length of 892.45 in 145 ms.

Scenario 2 (U-Shape Trap): Effectively escaped the trap (Local Minima) in the fastest time of 98 ms (Path Length: 645.32).

Scenario 3 (Staggered Walls): Handled lateral movement smoothly in 112 ms (Path Length: 723.18).
